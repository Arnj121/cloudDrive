...
function Login(){
    ...
    xhr.onreadystatechange = function(){
        if(xhr.readyState==4){
            ...
            if(res['status']) {
                ...
                document.cookie=`loggedin=1;expires=${d};path=/mydrive;`
                document.cookie = `name=${res['name']};expires=${d};path=/mydrive;`;
                document.cookie = `token=${res['token']};expires=${d};path=/mydrive;`
                document.cookie = `email=${res['email']};expires=${d};path=/mydrive;`
                document.cookie = `password=${res['password']};expires=${d};path=/mydrive;`
                location.replace(`http://drive.rainycloud.com:2000/mydrive`)
            }
            else{
    ...
    xhr.open('GET',`http://drive.rainycloud.com:2000/login?email=${email}&password=${password}`)
    xhr.send()
}
...
function signup(){
    ...
    xhr.open('POST',`http://localhost:2000/signup?email=${email}&password=${password}&name=${name}`)
    xhr.send()
}
