...
function getnewshares(t=0) {
    ...
    let url = `http://localhost:2000/getsharefile?token=${userdetails['token']}&only=only`
    if(t==1)
        url = `http://localhost:2000/getsharefile?token=${userdetails['token']}&only=notonly`
    xhr.open('GET',url)
    xhr.send()
}
function initmenu() {
...
function initspace(){
    ...
    xhr.open('GET',`http://localhost:2000/getspace?email=${userdetails['email']}`)
    xhr.send()
}
...
document.getElementById('space-info').onclick = ()=>{
    ...
    xhr.open('GET',`http://localhost:2000/spaceanalysis?token=${userdetails['token']}`)
    xhr.send()
}
...
document.getElementById('show-added-people').onclick = ()=>{
    ...
    xhr.open('GET',`http://localhost:2000/getpeople?token=${userdetails['token']}`)
    xhr.send()
}
...
document.getElementById('remove-more').onclick = ()=>{
    if(peopleselct!=0){
        ...
        xhr.open('DELETE',`http://localhost:2000/removepeople?token=${userdetails['token']}&email=${email}`)
        xhr.send()
    }
}
...
document.getElementById('search').oninput = (e)=>{
    ...
    if(val.length>=3){
        ...
        xhr.open('GET',`http://localhost:2000/search?search=${val}&token=${userdetails['token']}&searchloc=${searchparam['searchloc']}&owner=${searchparam['owner']}&email=${searchparam['email']}&searchType=${searchparam['search-type']}`)
        xhr.send()
    }
    else{
...
function RelaodWithFilter() {
    ...
    if (filterlist.length != 0) {
        ...
        let url = `http://localhost:2000/reloadwithfilter?filterlist=${filterlist}&cwdstring=${cwdstring}&token=${userdetails['token']}`
        if(masking=='Bin')
            url = `http://localhost:2000/reloadwithfilter?filterlist=${filterlist}&cwdstring=${cwdstring}&token=${userdetails['token']}&bin=1`
        else if(masking =='Favorites')
            url = `http://localhost:2000/reloadwithfilter?filterlist=${filterlist}&cwdstring=${cwdstring}&token=${userdetails['token']}&fav=1`
        else if(masking == 'shared with me')
            url = `http://localhost:2000/reloadwithfilter?filterlist=${filterlist}&cwdstring=${cwdstring}&token=${userdetails['token']}&swm=1`
        else if(masking == 'files-shared')
            url = `http://localhost:2000/reloadwithfilter?filterlist=${filterlist}&cwdstring=${cwdstring}&token=${userdetails['token']}&fs=1`
        xhr.open('GET',url)
        xhr.send()
    } else {
        cleanUp()
    ...
    if (selectfile == 1) {
        for (let i = 0; i < selectedfilelist.length; i++) {
            ...
            if (rndids[selectedfilelist[i]]['type'] == 'file')
                url = `http://localhost:2000/deletefile?filename=${rndids[selectedfilelist[i]]['name']}&token=${info[selectedfilelist[i]]['owner']}&cwdstring=${cwdstring}`
            else if (rndids[selectedfilelist[i]]['type'] == 'image')
                url = `http://localhost:2000/deleteimage?filename=${rndids[selectedfilelist[i]]['name']}&token=${info[selectedfilelist[i]]['owner']}&cwdstring=${cwdstring}`
            else
                url = `http://localhost:2000/deletefolder?filename=${rndids[selectedfilelist[i]]['name']}&token=${userdetails['token']}&cwd=${cwd}&cwdstring=${cwdstring}`
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
...
document.getElementById('username-ok').onclick = ()=>{
    ...
    if(newusername.length!=0){
        ...
        xhr.open("PUT",`http://localhost:2000/edituser?type=name`)
...
document.getElementById('password-ok').onclick = ()=>{
    ...
    if(newpassword.length!=0){
        ...
        xhr.open("PUT",`http://localhost:2000/edituser?type=password`)
        xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
    ...
    for(let i=0;i<Math.min(8,filenamekeys.length);i++){
        ...
        p.addEventListener('click',(e)=>{
            if(searchredirest[e.target.id]['type'] == 'file'){
                ...
                xhr.open('GET',`http://localhost:2000/loadfile?filename=${searchredirest[e.target.id]['name']}&token=${searchredirest[e.target.id]['path']}&cwdstring=${e.target.id}`)
                xhr.send()
            }
...
function initCollections() {
    if(userdetails['token'].length!=0) {
            ...
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (let i = 0; i < res.length; i++) {
                    ...
                    icon.onclick = (e)=>{
                        ...
                        xhr.open('POST',`http://localhost:2000/removecol`)
                        xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
        ...
        xhr.open('GET', `http://localhost:2000/getcollection?token=${userdetails['token']}`)
        xhr.send()
    }
}
function inituserdetails() {
    ...
    if(userdetails['token'] ==0 || userdetails['name']==0){
        location.replace('http://drive.rainycloud.com:2000/')
    }
...
function logout() {
    ...
    xhr.open('GET',`http://localhost:2000/logout?token=${userdetails['token']}`)
    xhr.send()
}
...
document.getElementById('add-people-confirm').onclick = ()=>{
    ...
    xhr.open('POST',`http://localhost:2000/addpeople?token=${userdetails['token']}`)
    xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
document.getElementById('create-collection-confirm').onclick = () =>{
    ...
    xhr.open('POST',`http://localhost:2000/createcollection?name=${name}&token=${userdetails['token']}`)
    xhr.send()
}
...
document.getElementById('file-upload-confirm').onclick = ()=>{
    ...
    for(let i=0;i<filesSelectedkeys.length;i++){
        ...
        fetch(`http://localhost:2000/uploadfile?token=${userdetails['token']}&cwd=${cwd}&lastmod=${filesSelected[filesSelectedkeys[i]].lastModifiedDate.toString().slice(0,24)}&cwdstring=${cwdstring}`,{method:'POST',body:formdata})
...
document.getElementById('image-upload-confirm').onclick = ()=>{
    ...
    for(let i=0;i<imagesSelectedkeys.length;i++){
        ...
        fetch(`http://localhost:2000/uploadimage?token=${userdetails['token']}&cwdstring=${cwdstring}&lastmod=${imagesSelected[imagesSelectedkeys[i]].lastModifiedDate.toString().slice(0,24)}&cwdstring=${cwdstring}`,{method:'POST',body:formdata})
           .then(res=>console.log(res))
    }
    let filename;
    setTimeout(()=>{
        ...
        for(let i=0;i<imagesSelectedkeys.length;i++){
            ...
            xhr.open('GET',`http://localhost:2000/loadimage?filename=${filename}&token=${userdetails['token']}&cwdstring=${cwdstring}`)
            xhr.send()
        }
        setTimeout(()=>{
...
function createFolder(){
    ...
    xhr.open('POST',`http://localhost:2000/createfolder?name=${name}&token=${userdetails['token']}&cwd=${cwd}&cwdstring=${cwdstring}`)
    xhr.send()
...
document.getElementById('save-file').onclick = ()=>{
    ...
    xhr.open('POST',`http://localhost:2000/createfile?token=${userdetails['token']}&data=${data}&name=${name}&lastmod=${lastmod}&cwdstring=${cwdstring}&size=${data.length}`)
    xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
document.getElementById('delete').onclick=(e)=>{
    ...
    if(rndids[currentselected]['type'] == 'file')
        url = `http://localhost:2000/deletefile?filename=${rndids[currentselected]['name']}&token=${info[currentselected]['owner']}&cwdstring=${cwdstring}`
    else if(rndids[currentselected]['type'] == 'image' || rndids[currentselected]['type'] == 'video' || rndids[currentselected]['type'] == 'audio')
        url = `http://localhost:2000/deleteimage?filename=${rndids[currentselected]['name']}&token=${info[currentselected]['owner']}&cwdstring=${cwdstring}`
    else
        url = `http://localhost:2000/deletefolder?filename=${rndids[currentselected]['name']}&token=${userdetails['token']}&cwd=${cwd}&cwdstring=${cwdstring}`
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4){
            //remove the element from dom
...
document.getElementById('rename-confirm').onclick = ()=>{
    ...
    let xhr = new XMLHttpRequest()
    let url = `http://localhost:2000/updatefilename?oldname=${rndids[currentselected]['name']}&newname=${newfilename}&token=${userdetails['token']}&cwdstring=${cwdstring}`
    if(rndids[currentselected]['type'] == 'image')
        url = `http://localhost:2000/updateimagename?oldname=${rndids[currentselected]['name']}&newname=${newfilename}&token=${userdetails['token']}&cwdstring=${cwdstring}`
    else if(rndids[currentselected]['type'] == 'dir')
        url = `http://localhost:2000/updatefoldername?oldname=${rndids[currentselected]['name']}&newname=${newfilename}&token=${userdetails['token']}&cwdstring=${cwdstring}`
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4){
...
document.getElementById('download').onclick = ()=>{
    ...
    window.location.assign(`http://localhost:2000/download?name=${file}&token=${userdetails['token']}&type=${info[currentselected]['type']}`)
    document.getElementById(currentselected).click()
    currentselected=0
}
...
document.getElementById('fav').onclick = ()=>{
    ...
    xhr.open('POST',`http://localhost:2000/addfav?token=${userdetails['token']}&cwdstring=${cwdstring}&filename=${filename}&type=${type}`)
    xhr.send()
}
...
document.getElementById('add-confirm').onclick = ()=>{
    ...
    xhr.open('POST',`http://localhost:2000/addtocollection?cname=${cname}&type=${type}&name=${name}&token=${userdetails['token']}&cwdstring=${cwdstring}`)
    xhr.send()
}
...
function shareFile(){
    ...
    xhr.open('POST',`http://localhost:2000/sharefile?select=${select}&email=${email}&token=${userdetails['token']}&cwdstring=${cwdstring}`)
    xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
document.getElementById('paste').onclick = ()=>{
    ...
    if(filename.length!=0) {
        ...
        xhr.open('POST', `http://localhost:2000/transfer?oldcwdstring=${oldcwdstring}&newcwdstring=${newcwdstring}&token=${userdetails['token']}&status=${status}&type=${type}&filename=${filename}`)
        xhr.send()
    }
...
document.getElementById('edit-file').onclick =()=>{
    if(editmode){
        ...
        xhr.open('POST',`http://localhost:2000/savefile?token=${userdetails['token']}`)
        xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
        ...
        document.getElementById(id).ondblclick = (e) => {
            if(selectfile ==0) {
                ...
                if (rndids[e.target.id]['type'] == 'file') {
                    ...
                    let url = `http://localhost:2000/loadfile?filename=${filename}&token=${info[e.target.id]['owner']}&cwdstring=${cwdstring}`
                    let xhr = new XMLHttpRequest()
                    let data = ''
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState == 4) {
...
function getFavfiles(){
    ...
    xhr.open('GET', `http://localhost:2000/getfav?token=${userdetails['token']}`)
    xhr.send()
}
...
function getsharefiles() {
    ...
    xhr.open('GET',`http://localhost:2000/getsharefile?token=${userdetails['token']}&only=all`)
    xhr.send()
}
function getCollecFiles() {
    ...
    xhr.open('GET',`http://localhost:2000/listcollection?token=${userdetails['token']}&cname=${curcolname}`)
    xhr.send()
}
...
function getfilesshared() {
    ...
    xhr.open('GET',`http://localhost:2000/getfilesshared?token=${userdetails['token']}`)
    xhr.send()
}
...
function getBinFiles() {
    ...
    xhr.open('GET',`http://localhost:2000/listbinfiles?token=${userdetails['token']}&bin=1`)
    xhr.send()
}
function showTrashIcon() {
    ...
    for(let i=0;i<rndidskeys.length;i++){
        ...
        ele.onclick = (e)=>{
            ...
            xhr.open('POST',`http://localhost:2000/restorefile?token=${userdetails['token']}`)
            xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
function showStopSharingicon() {
    ...
    for(let i=0;i<rndidskeys.length;i++){
        ...
        ele.onclick = (e)=>{
            ...
            xhr.open('POST',`http://localhost:2000/stopsharing?token=${userdetails['token']}`)
            xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
function showremovefavicon(){
    ...
    for(let i=0;i<keys.length;i++){
        ...
        ele.onclick = (e)=>{
            ...
            xhr.open('POST','http://localhost:2000/removefav')
            xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
function  showremovefromcol() {
    ...
    for(let i=0;i<keys.length;i++){
        ...
        ele.onclick = (e)=>{
            ...
            xhr.open('POST','http://localhost:2000/removefromcol')
            xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8')
...
function getFiles() {
    if(masking == 'Favorites'){
        ...
        xhr.open('GET', `http://localhost:2000/listallfiles?token=${userdetails['token']}&cwd=${cwd}&cwdstring=${cwdstring}`)
        xhr.send()
    }
}
